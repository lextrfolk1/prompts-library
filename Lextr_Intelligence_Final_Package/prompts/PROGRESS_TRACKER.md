...
**Current State:** Working on `feature/lextr-intelligence-v1.38.0`, LP by LP: each prompt verified against the base implementation, gaps filled additively (nothing removed). 251/251 DELIVERED (LP-27.1/27.2/28.1/28.2 as design docs, no code), 0 IN_PROGRESS, 0 PENDING. Gates/tests NOT RUN (final pass). DB scripts vs code audited on scratch PG17 2026-09-25 (section 6, db-audit rows); next migration V44. Per-change log in section 6. Last updated 2026-09-25.  
...
| db-audit | DB scripts vs code (svc V1..V41 + queries.properties + inline SQL; lexie variance 001..010 + DDL/DML + Python SQL), verified on scratch PG17+pgvector: every migration applied, every statement PREPAREd, inserts vs NOT NULL, mapper columns, named params, enum labels | 154/174 catalogue queries glued by Properties continuation (syntax error at first call); TenantDaoImpl 11 unqualified tables; tenant_id NOT NULL never written + FORCE RLS on a GUC never set (non-superuser reads 0 rows, inserts fail); output_type rule_draft unreadable in Java | separators before 919 continuations; tenant SQL schema-qualified; RULE_DRAFT; QueryCatalogIntegrityTest (glue + inline-schema gates w/ controls); V42 RLS off per owner decision (isolation in OPA, V1 contract) + inserts write tenant_id = client_id | svc 355890a, 956b5da |
| db-audit | lexie-ai variance DB | demo_generated_period created only by runtime DDL in the route; runner globbed *.sql and stopped with 'content changed' on every pipeline-built DB (DDL records marker hash) | 011_demo_generated_period.sql; runner reads NNN_*.sql only (numbered == DDL+DML verified: same schema, same seed rows) | lexie 1a9f4a2 |
| db-audit | reported, not changed | Java RunStatus PENDING/NEEDS_INPUT have no run_status label (not written today); lexie STOPPED/IN_PROGRESS map to null -> persisted as completed; DefinitionKind.DROP_PROFILE not a definition_kind label (?kind=drop_profile filter would 500); suggestion_feedback (legacy MDRM service) has no DDL anywhere; defined-but-unused tables chain_discontinuity, evidence_ledger_marker, tenant_key_registry | owner decisions | — |
| db-audit | tenant_id vs client_id | V14 tenant_id was a copy of client_id on 4 tables, read by nothing | V43 (owner decision): dropped on agent_run / preset / registered_definition with tenant_profile FK moved to client_id; kept on agent_run_step (inside every evidence content_hash) with CHECK (tenant_id = client_id) | svc 8c18c0a |
| db-migrate | real Flyway 10.10.0 run (Spring config) on scratch PG17: fresh superuser, fresh non-superuser owner, V1+data and V13+data upgrades, DB at V43 (validate + no-op re-run); variance runner + pipeline paths as non-superuser and 003+data upgrade; PG17+/18-only syntax scan (none) | V14 tenant FK stopped any upgrade with data for a client other than the V13 seeds; pgvector untrusted -> non-superuser owner stops at V1; V4 fails on pre-existing duplicate operational presets | beforeEachMigrate callback registers existing clients only between V13 and V14 (V14 is on origin: not edited); README: DBA creates vector first + V4 pre-check query (V4 left to a human: retiring a live preset); BaselineSchemaMigrationTest pins the callback | svc c80dcb9 |
| db-migrate | dev DB (application-dev.yaml, [::1]:5433/lextr) migrated for real with Flyway 10.10.0 | first attempt refused: role search_path 'intelligence, meta, data, public' made Flyway use the non-empty platform schema meta (service could not have started) | spring.flyway.default-schema=public (reproduced + fixed on scratch first); dev DB now at V43: 43 applied, validate ok, 55 intelligence tables, nothing else created besides public.flyway_schema_history (+ pgcrypto extension in public from a migration); 174 + 16 statements PREPARE on it | svc aba46ae |
| run | intelligence-service + intelligence-ui started in dev against the dev DB | startup blocked by: stale target/ migrations (mvn clean), 3 beans with two un-annotated constructors, SemanticLayerClient with no implementation; unmapped route -> 500; UI dev calls hit Vite (CORS 403 if pointed at :8059) | Flyway REMOVED from the service (owner decision; scripts applied outside, README has vector prerequisite + pre-V4/pre-V14 SQL); @Autowired on the production constructors; UnboundSemanticLayerClient (fail-closed); 404 ROUTE_NOT_FOUND; Vite /api proxy. Service up on :8059 (liveness UP, readiness DOWN: OPA/lexie not running), UI on :5173/intelligence/; LP-47 gates GREEN | svc 93807a0, ui ca92a63 |
| run | OPA wiring checked live | evaluatePolicy returned null on no decision: analytical / digital-twin / impact / trend ran WITHOUT a policy check when OPA was down (fail-open), 14 other sites NPE'd; bulk policy refresh uploaded alphabetically (capability before common -> 400) and still reported it loaded; policies are never loaded automatically | no decision = explicit deny OPA_UNAVAILABLE (all 28 sites fail closed); refresh retries until no progress and reports failedPolicies; local OPA 1.4.2 (--v0-compatible) on :8181 with 46/46 policies + 12 data docs; allow and deny observed through the service | svc 9736677 |
